...
// post req for marking the attendance (daily or period wise both)
...
//         // 6. if attendence is already marked update the document
//         if (attendanceDoc) {
...
//         });
//         if(!newAttendance) {
//             throw new ApiError("New attendance didn't get created", 500)
//         }
...
export async function POST(req) {
    try {
        ...
        if (attendanceDoc) {
            attendanceDoc.students = records.map((rec) => {
                return {
